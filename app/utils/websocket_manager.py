...
class WebSocketManager:
    ...
    async def connect(self, websocket: WebSocket, session_id: str):
        try:
            ...
            self._ping_tasks[session_id] = asyncio.create_task(
                self._ping_task(websocket, session_id)
            )

            # Start connection timeout task
            self._timeout_tasks[session_id] = asyncio.create_task(
                self._connection_timeout_task(websocket, session_id)
            )

        except Exception as e:
            logger.error(
                f"Error connecting WebSocket for session {session_id}: {str(e)}"
            )
            raise
    ...
    async def _connection_timeout_task(self, websocket: WebSocket, session_id: str):
        try:
            ...
            logger.error(
                f"Error in connection timeout task for session {session_id}: {str(e)}"
            )
        finally:
                ...
                try:
                    ...
                    try:
                        data = await asyncio.wait_for(
                            websocket.receive_text(), timeout=self.ping_timeout
                        )
                        if data != "pong":
                            logger.warning(
                                f"Invalid response to ping from session {session_id}"
                            )
                            break
                    ...
                    logger.info(
                        f"WebSocket disconnected during ping for session {session_id}"
                    )
                    break
                except Exception as e:
                    logger.error(
                        f"Error sending ping to session {session_id}: {str(e)}"
                    )
                    break
        except Exception as e:
    ...
    def disconnect(self, websocket: WebSocket, session_id: str):
        try:
            ...
            logger.error(
                f"Error disconnecting WebSocket for session {session_id}: {str(e)}"
            )
        ...
        if session_id in self.active_connections:
            ...
            for connection in self.active_connections[session_id]:
                try:
                    ...
                    logger.info(
                        f"WebSocket disconnected during broadcast for session {session_id}"
                    )
...
class WebSocketLogHandler(logging.Handler):
    ...
    def emit(self, record):
        try:
            ...
            asyncio.create_task(
                self.manager.broadcast_log(self.session_id, msg, record.levelname)
            )
        except Exception as e:
